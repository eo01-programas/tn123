(() => {
    ...
    function applyClientFilterToView(viewId = state.activeView) {
        ...
        for (const [colLabel, filterValue] of Object.entries(viewColumnFilters)) {
            ...
            const colIdx = getTableColumnIndexByHeader(table, colLabel);
            if (colIdx >= 0) {
                activeFilters.push({ colIdx, key: normalizeClientFilterKey(filterValue) });
            }
        }
        ...
        rows.forEach((row) => {
            const exactMatches = activeFilters.every(({ colIdx, key }) => {
                const cellValue = getColumnCellValue(row.cells[colIdx]);
                return normalizeClientFilterKey(cellValue) === key;
            });
