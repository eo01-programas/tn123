...
`.trim();
    ...
    // Lab Tinto: la edicion depende del subtab activo y del usuario.
    //   Por pasar muestra (MUESTRA): solo Asis_tintoreria y Pcp_textil editan rb/maq/status.
    //   Por receta (RECETA) y Receta OK (OK): solo lab_tintoreria edita esos campos.
    // Los demas usuarios ven los controles como datos (deshabilitados, sin poder modificar).
    const LAB_TINTO_VIEW_ID = 'lab-tinto';
    const LAB_TINTO_SUBTAB_EDITORS = {
        MUESTRA: ['Asis_tintoreria', 'Pcp_textil'],
        RECETA: ['lab_tintoreria'],
        OK: ['lab_tintoreria']
    };
    // Solo estos usuarios pueden registrar nuevas solicitudes con "Solicita Receta".
    const LAB_TINTO_SOLICITA_USERS = ['Pcp_textil'];
    ...
    const PERMISSION_PROFILES = {
        ...
        Supervisor01: {
            ...
            programadoOnlyViews: ['plegado', 'rama-crudo', 'abridora', 'secado', 'rama-tenido', 'acab-espec'],
            alwaysEditableViews: [],
            preserveControlsDisabled: true,
        ...
        Supervisor02: {
            canAccessMaestro: false,
            fullAccess: false,
            readOnlyViews: ['plegado', 'rama-crudo', 'abridora', 'secado', 'rama-tenido', 'acab-espec', 'calidad', 'embalaje'],
        ...
        Calidad: {
            canAccessMaestro: false,
            fullAccess: false,
            readOnlyViews: ['plegado', 'rama-crudo', 'preparado', 'tenido', 'abridora', 'secado', 'rama-tenido', 'acab-espec', 'embalaje'],
            programadoOnlyViews: [],
        ...
        Embalaje: {
            canAccessMaestro: false,
            fullAccess: false,
            readOnlyViews: ['plegado', 'rama-crudo', 'preparado', 'tenido', 'abridora', 'secado', 'rama-tenido', 'acab-espec', 'calidad'],
            programadoOnlyViews: [],
        ...
        lab_tintoreria: {
            ...
            readOnlyViews: ['plegado', 'rama-crudo', 'preparado', 'tenido', 'abridora', 'secado', 'rama-tenido', 'acab-espec', 'calidad', 'embalaje'],
            programadoOnlyViews: [],
            // La edicion de Lab Tinto se resuelve por subtab en LAB_TINTO_SUBTAB_EDITORS.
            alwaysEditableViews: [],
            preserveControlsDisabled: true,
            defaultView: 'lab-tinto',
            defaultFilter: ''
    ...
    function buildSession(username) {
        ...
        return {
            ...
            preserveControlsDisabled: Boolean(profile.preserveControlsDisabled),
    ...
    function canEditLabTintoSubtab(username, activeFilter) {
        const subtab = String(activeFilter || '').trim().toUpperCase();
        const editors = LAB_TINTO_SUBTAB_EDITORS[subtab];
        return Array.isArray(editors) && editors.includes(username);
    }

    function canSolicitaReceta() {
        return Boolean(currentSession) && LAB_TINTO_SOLICITA_USERS.includes(currentSession.username);
    }
    ...
    function canEditView(viewId, activeFilter = '') {
        ...
        // Lab Tinto se rige por la matriz subtab/usuario, incluso para cuentas full-access.
        if (viewId === LAB_TINTO_VIEW_ID) {
            return canEditLabTintoSubtab(currentSession.username, activeFilter);
        }
