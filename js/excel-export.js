...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <tableStyles count="0" defaultTableStyle="TableStyleMedium2" defaultPivotStyle="PivotStyleLight16"/>
    ...
    function buildWorksheetXml(sheet) {
        ...
        // Fila de titulo opcional (ej. "PROCESO: Abridora - Impresion ...")
        // como fila 1 combinada a lo ancho; desplaza encabezado y datos una
        // fila hacia abajo y se repite al imprimir via Print_Titles.
        const titleOffset = sheet.title ? 1 : 0;
        const headerRowNumber = 1 + titleOffset;

        let titleRowXml = '';
        if (sheet.title) {
            const titleCellsXml = sheet.columns.map((column, columnIndex) => {
                if (columnIndex === 0) {
                    return `<c r="A1" s="21" t="inlineStr">${buildInlineString(sheet.title)}</c>`;
                }
                return `<c r="${columnName(columnIndex)}1" s="21"/>`;
            }).join('');
            titleRowXml = `<row r="1" spans="1:${sheet.columns.length}" ht="24" customHeight="1">${titleCellsXml}</row>`;
            if (sheet.columns.length > 1) {
                mergeRefs.push(`A1:${lastColumn}1`);
            }
        }

        const headerRow = `<row r="${headerRowNumber}" spans="1:${sheet.columns.length}" ht="22" customHeight="1">${
            sheet.columns.map((column, columnIndex) => (
                `<c r="${columnName(columnIndex)}${headerRowNumber}" s="1" t="inlineStr">${buildInlineString(column.header)}</c>`
            )).join('')
        }</row>`;

        const bodyRows = sheet.rows.map((row, rowIndex) => {
            const rowNumber = rowIndex + headerRowNumber + 1;
        ...
        const lastDataRow = Math.max(headerRowNumber, sheet.rows.length + headerRowNumber);
        const tableRef = `A${headerRowNumber}:${lastColumn}${lastDataRow}`;

        // Nota final (negritas) que tambien aparece al imprimir.
        let footerNoteRowXml = '';
        let lastSheetRow = lastDataRow;
        if (sheet.footerNote) {
            const noteRowNumber = lastDataRow + 1;
            ...
            if (sheet.columns.length > 1) {
                mergeRefs.push(`A${noteRowNumber}:${lastColumn}${noteRowNumber}`);
            }
        }
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <sheetData>${titleRowXml}${headerRow}${bodyRows}${footerNoteRowXml}</sheetData>
  ...
  ${mergeCellsXml}${printXml}${rowBreaksXml}
    ...
    function buildWorkbookXml(sheets) {
        ...
        // Print_Titles: repite el encabezado (y la fila de titulo si existe)
        // en todas las hojas impresas.
        const printTitles = sheets.map((sheet, index) => {
            if (!sheet.repeatHeader) {
                return '';
            }
            const ref = `${quoteSheetNameForRef(sheet.name)}!$1:$${sheet.title ? 2 : 1}`;
  ...
  <dcterms:modified xsi:type="dcterms:W3CDTF">${timestamp}</dcterms:modified>
    ...
    function createStyledWorkbookBlob(options = {}) {
        ...
        const normalizedSheets = requestedSheets.map((sheet) => {
            ...
            const title = sheet && sheet.title ? String(sheet.title) : '';
            ...
            return {
                ...
                rowBreaks: normalizeRowBreaks(sheet && sheet.rowBreaks, rows.length + (title ? 2 : 1)),
