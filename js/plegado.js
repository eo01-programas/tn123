(() => {
    ...
    function renderSubtabCounts(records) {
        ...
        document.getElementById('count-plegado-prog').textContent = `${new Set(progRecords.map((r) => TintoreriaUtils.formatOpPartida(r.op_tela, r.partida))).size} ptds`;
    ...
    function renderTable(records, state) {
        ...
        tbody.innerHTML = filtered.map((record) => `
            <tr${TintoreriaUtils.isUrgentPriority(record.plegado_p) ? ' class="urgent-row"' : ''}>
                ...
                <td>
                    <select class="table-select" ${readOnly ? 'tabindex="-1" style="pointer-events:none;"' : `data-record-id="${TintoreriaUtils.escapeHtml(record.id_registro)}" data-field="plegado_turno"`}>
