(() => {
    ...
    // P | cliente | OP-PTDA | color | articulo | kg | Tipo | Turno | MAQ | Status
    const ACAB_ESPEC_WIDTHS = [36, 50, 71, 112, 146, 78, 134, 70, 65, 70];
    ...
    function hasSelectedSpecialType(record) {
        ...
        return Boolean(tipo) && !isNoLlevaSpecialType(record);
    }

    function getEligibleRecords(records) {
        return records.filter((record) => {
            if (isNoLlevaSpecialType(record)) {
                return false;
            }
    ...
    function renderSubtabCounts(records) {
        ...
        document.getElementById('count-acab-espec-xprog').textContent = `${new Set(xprogRecords.map((r) => TintoreriaUtils.formatOpPartida(r.op_tela, r.partida))).size} ptds`;
        document.getElementById('count-acab-espec-prog').textContent = `${new Set(progRecords.map((r) => TintoreriaUtils.formatOpPartida(r.op_tela, r.partida))).size} ptds`;
        ...
        if (colgroup) colgroup.innerHTML = ACAB_ESPEC_WIDTHS.map(w => `<col style="width:${w}px">`).join('');
    }
