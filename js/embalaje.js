(() => {
    ...
    function handleEmbalajeContextMenu(event) {
        ...
        // kg(crudo) column (index 7) — devolver a Calidad, only for Calidad user
        if (cellIndex === 7 && isCalidadUser()) {
        ...
        if (!filtered.length) {
            tbody.innerHTML = `
                <tr class="empty-state">
                    <td colspan="10">No hay filas visibles en Embalaje.</td>
                </tr>
            `;
