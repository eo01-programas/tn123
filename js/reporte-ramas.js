(() => {
    ...
    function accumulateRecord(record, dayMap, chartEvents, dayRecordsIndex) {
        ...
        function addToChart(dateValue, turno, maquina) {
            ...
            if (d) chartEvents.push({ date: d, kg, turno, maquina, opPartida, cliente });
        }
    ...
    // Filtro de cliente por gráfico ('Todos' = sin filtro).
    const TODOS_CLIENTES = 'Todos';
    const chartClientes = { ramas: TODOS_CLIENTES, embalaje: TODOS_CLIENTES };

    function filterByCliente(events, cliente) {
        if (cliente === TODOS_CLIENTES) return events;
        return events.filter(ev => (ev.cliente || '—') === cliente);
    }

    function renderRamasChart() {
        const events = filterByCliente(ramasChartEvents, chartClientes.ramas);
        chartSeries.ramas = buildChartSeries(events, chartPeriods.ramas);
        renderBarChart('rr-weekly-chart', chartSeries.ramas);
    }

    function renderEmbalajeChart() {
        const events = filterByCliente(embalajeChartEvents, chartClientes.embalaje);
        let data = buildChartSeries(events, chartPeriods.embalaje);
    ...
    function renderDetailHead(showTurno, showMaquina, showCalidad) {
        return `
            ...
            ${showCalidad ? '<th class="rr-detail-sep" style="width: 44px; text-align:center;">#R</th>' : ''}
            ${showCalidad ? '<th class="rr-detail-sep" style="width: 130px;">Quien aprobo</th>' : ''}
        `;
    }

    // Mismas píldoras AC/LV que la columna Ruta_tela de Calidad
    // (calidad.js renderRutaTelaCell).
    function renderRutaTelaMarkup(record) {
        const ruta = String(record.ruta_tela_final || '').trim().toUpperCase();
        if (ruta === 'ACABADA') {
            return '<span class="process-pill ruta-tela-pill ruta-tela-pill-ac" title="ACABADA">AC</span>';
        }
        if (ruta === 'LAVADA') {
            return '<span class="process-pill ruta-tela-pill ruta-tela-pill-lv" title="LAVADA">LV</span>';
        }
        return '<span class="rr-empty">—</span>';
    }

    // Número entero de rechazos acumulados; "-" cuando la partida no tuvo ninguno.
    function renderRechazosMarkup(record) {
        const count = parseInt(record.cantidad_rechazos, 10);
    ...
    function renderDetailRows(items, showQualityEye, showTurno, showMaquina, showCalidad) {
        ...
        return sorted.map(item => {
            ...
            <tr class="${groupClass}">
                ...
                ${showCalidad ? `<td class="rr-detail-sep" style="text-align:center">${TintoreriaUtils.escapeHtml(renderRechazosMarkup(record))}</td>` : ''}
                ${showCalidad ? `<td class="rr-detail-sep">${String(record.quien_aprobo || '').trim()
                    ? `<span class="cell-text">${TintoreriaUtils.escapeHtml(String(record.quien_aprobo).trim())}</span>`
                    : '<span class="rr-empty">—</span>'}</td>` : ''}
            </tr>
        `;
        }).join('');
    }
    ...
    // ── dropdown flotante de cliente por gráfico ─────────────────────────────

    // Clientes principales primero en el desplegable, en este orden; el resto
    // va después en orden alfabético. Se incluye ATHELTA/ATHLETA por si la
    // hoja registra cualquiera de las dos grafías.
    const CLIENTE_PRIORITY = ['LULU', 'BANANA', 'ATHELTA', 'ATHLETA', 'THEORY', 'SKECHERS'];

    function clientePriority(name) {
        const idx = CLIENTE_PRIORITY.indexOf(String(name).trim().toUpperCase());
        return idx === -1 ? CLIENTE_PRIORITY.length : idx;
    }

    // Repuebla las opciones con los clientes únicos presentes en los eventos de
    // cada gráfico; si el cliente seleccionado ya no existe, vuelve a "Todos".
    function renderClienteMenus() {
        [
            { chart: 'ramas', events: ramasChartEvents },
            { chart: 'embalaje', events: embalajeChartEvents }
        ].forEach(({ chart, events }) => {
            const menu = document.querySelector(`.rr-chart-menu[data-chart-cliente="${chart}"]`);
            if (!menu) return;
            const label = menu.querySelector('.rr-cliente-label');
            const list = menu.querySelector('.rr-chart-menu-list');

            const clientes = [...new Set(events.map(ev => ev.cliente || '—'))]
                .sort((a, b) => clientePriority(a) - clientePriority(b) || a.localeCompare(b, 'es'));
            if (chartClientes[chart] !== TODOS_CLIENTES && !clientes.includes(chartClientes[chart])) {
                chartClientes[chart] = TODOS_CLIENTES;
            }

            if (list) {
                list.innerHTML = [TODOS_CLIENTES, ...clientes].map(cliente => {
                    const active = cliente === chartClientes[chart] ? ' active' : '';
                    const escaped = TintoreriaUtils.escapeHtml(cliente);
                    return `<button type="button" class="rr-chart-menu-option${active}" data-cliente="${escaped}">${escaped}</button>`;
                }).join('');
            }
            if (label) label.textContent = chartClientes[chart];
        });
    }

    function bindClienteMenus() {
        document.querySelectorAll('.rr-chart-menu[data-chart-cliente]').forEach(menu => {
            const chart = menu.dataset.chartCliente;
            const button = menu.querySelector('.rr-chart-menu-btn');
            const list = menu.querySelector('.rr-chart-menu-list');
            if (!chart || !button || !list) return;
            ...
            // Delegación: las opciones se regeneran en cada render.
            list.addEventListener('click', (event) => {
                const option = event.target.closest('.rr-chart-menu-option');
                if (!option) return;
                event.stopPropagation();
                chartClientes[chart] = option.dataset.cliente;
                renderClienteMenus();
                list.classList.add('hidden');
                if (chart === 'ramas') renderRamasChart();
                else renderEmbalajeChart();
