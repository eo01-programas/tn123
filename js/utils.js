(() => {
    ...
    // Pases acumulados por las apps mobile: valores separados por coma en un mismo campo
    // (ej. preparado_inicio = "05/Jul/2026 08:15am,07/Jul/2026 09:30am").
    function parsePassValues(fieldValue) {
        const raw = String(fieldValue === undefined || fieldValue === null ? '' : fieldValue).trim();
        if (!raw) {
            return [];
        }
    ...
    function lastPassValue(fieldValue) {
        const values = parsePassValues(fieldValue).filter(Boolean);
    ...
    const SPANDEX_ARTICLE_KEYWORDS = ['SPANDEX', 'SPDX', 'SPAND1:1', 'SPAND', 'SPAN', 'SPD', 'SP', 'PES'];
    ...
    function normalizeClienteName(value) {
        ...
        if (upper === 'AM RETAIL S.A.C.' || upper === 'AM RETAIL SAC' || upper === 'AM RETAIL S.A.C') {
            return 'AM RETAIL';
        }
        return raw || value;
    }
    ...
    function defaultRecord(record = {}) {
        ...
        if (merged.cliente) {
            merged.cliente = normalizeClienteName(merged.cliente);
        }
        return merged;
    }
    ...
    function loadXLSX() {
        ...
        _xlsxLoadPromise = new Promise((resolve, reject) => {
            ...
            script.onerror = () => {
                ...
                reject(new Error('No se pudo cargar la libreria XLSX. Verifica tu conexion.'));
            };
