(() => {
    ...
    function renderSubtabCounts(records) {
        ...
        document.getElementById('count-abridora-prog').textContent  = `${new Set(progRecords.map((r) => TintoreriaUtils.formatOpPartida(r.op_tela, r.partida))).size} ptds`;
    ...
    function renderTable(records, state) {
        ...
        tbody.innerHTML = filtered.map((record) => `
            <tr${TintoreriaUtils.isUrgentPriority(record.abridora_p) ? ' class="urgent-row"' : ''}>
                ...
                <td>
                    <select class="table-select" ${readOnly ? 'tabindex="-1" style="pointer-events:none;"' : `data-record-id="${TintoreriaUtils.escapeHtml(record.id_registro)}" data-field="abridora_turno"`}>
    ...
    TintoreriaApp.registerView('abridora', {
        ...
        locateRecord(record) {
            ...
            return {
                filter: normalizeAbridoraState(record) === 'OK' ? 'PROG' : 'X PROG'
            };
        }
    });
})();
