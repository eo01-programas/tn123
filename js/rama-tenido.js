(() => {
    ...
    // Por procesar (13): P | cliente | tela | OP-PTDA | color | cod_art | articulo | kg | Turno | MAQ | Proceso | Inicio | Status
    const RAMA_TENIDO_WIDTHS = [37, 72, 50, 108, 100, 90, 220, 68, 60, 72, 85, 75, 95];
    // Procesado (15): Fin | cliente | tela | OP-PTDA | color | cod_art | articulo | kg | Turno | Oper | MAQ | Proceso | Insp | Superv | Status
    const RAMA_TENIDO_WIDTHS_PROCESADO = [68, 52, 50, 99, 87, 79, 180, 62, 62, 87, 62, 81, 74, 74, 68];
    const DETAIL_FIELDS = [
        'rama_tenido_ancho',
    ...
    // ¿Tiene un pase abierto? (mas inicios que fines registrados por ramas_mobile)
    function hasOpenPass(record) {
        const inicios = TintoreriaUtils.parsePassValues(record.rama_tenido_inicio).filter(Boolean).length;
        const fines = TintoreriaUtils.parsePassValues(record.rama_tenido_fin).filter(Boolean).length;
    ...
    function getFilteredRecords(records) {
        ...
        if (currentFilter === 'PROG') {
            const sorted = eligible
                .filter((record) => ['PROG', 'OK'].includes(normalizeRamaTenidoState(record)))
                .sort((a, b) => {
                    const dateA = TintoreriaUtils.parseDateish(TintoreriaUtils.lastPassValue(a.rama_tenido_fin));
                    const dateB = TintoreriaUtils.parseDateish(TintoreriaUtils.lastPassValue(b.rama_tenido_fin));
                    if (!dateA && !dateB) return 0;
                    if (!dateA) return 1;
                    if (!dateB) return -1;
                    return dateB.getTime() - dateA.getTime();
                });
            return TintoreriaProcessedWindow.filterToWindow('rama-tenido', sorted, getProcessedDate);
        }
        ...
        return TintoreriaUtils.sortRecordsByPriority(
            ...
            const bHasInicio = Boolean(b.rama_tenido_inicio);
    ...
    function validateDetailValues(changes) {
        ...
        if (changes.rama_tenido_alimentacion && !TintoreriaUtils.isValidNumericRatio(changes.rama_tenido_alimentacion, 2)) {
            return 'rama_tenido_alimentacion admite 1 a 2 digitos o formato numero/numero.';
        }
    ...
    function renderSubtabCounts(records) {
        ...
        const progWindowed = TintoreriaProcessedWindow.filterToWindow('rama-tenido', progRecords, getProcessedDate);
        ...
        document.getElementById('count-rama-tenido-prog').textContent = `${new Set(progWindowed.map((r) => TintoreriaUtils.formatOpPartida(r.op_tela, r.partida))).size} ptds`;
        document.getElementById('summary-rama-tenido-xprog').textContent = TintoreriaUtils.formatSubtabSummary(xprogRecords);
        document.getElementById('summary-rama-tenido-prog').textContent = TintoreriaUtils.formatSubtabSummary(progWindowed);
    }

    function renderTheadForFilter() {
        const theadTr = document.getElementById('thead-tr-rama-tenido');
        const colgroup = document.getElementById('colgroup-rama-tenido');
        ...
        if (currentFilter === 'PROG') {
            theadTr.innerHTML = `
                ...
                <th>Status</th>
            `;
            colgroup.innerHTML = RAMA_TENIDO_WIDTHS_PROCESADO.map(w => `<col style="width:${w}px">`).join('');
        } else {
            theadTr.innerHTML = `
                <th>P</th>
                <th>cliente</th>
                ...
                <th>Status</th>
            `;
            colgroup.innerHTML = RAMA_TENIDO_WIDTHS.map(w => `<col style="width:${w}px">`).join('');
        }
        const rtTable = document.querySelector('table.rama-tenido-table');
        if (rtTable) {
            rtTable.classList.toggle('rama-tenido-xprog', currentFilter !== 'PROG');
            rtTable.classList.toggle('rama-tenido-prog', currentFilter === 'PROG');
        }
    }

    function getAdjustedRamaTenidoDate(rawValue) {
    ...
    function getProcessedDate(record) {
        return getAdjustedRamaTenidoDate(record ? TintoreriaUtils.lastPassValue(record.rama_tenido_fin) : null);
    }

    function renderRowProcesado(record) {
        const lastInicio = TintoreriaUtils.lastPassValue(record.rama_tenido_inicio);
        const lastFin = TintoreriaUtils.lastPassValue(record.rama_tenido_fin);
        const adjusted = getAdjustedRamaTenidoDate(lastFin);
        const finDate = TintoreriaUtils.formatDateDayMonth(adjusted) || '--';
        const inicioLabel = TintoreriaUtils.formatProcessDateTimeLabel(lastInicio) || '--';
        const finLabel = TintoreriaUtils.formatProcessDateTimeLabel(lastFin) || '--';
        const elapsed = TintoreriaUtils.formatElapsedTime(lastInicio, lastFin) || '--:--';
        ...
        return `
            <tr${TintoreriaUtils.isUrgentPriority(record.rama_tenido_p) ? ' class="urgent-row"' : ''}>
                ...
                <td>
                    <select class="table-select" tabindex="-1" style="pointer-events:none; appearance:none; -webkit-appearance:none;">
                        ${optionMarkup(TintoreriaUtils.lastPassValue(record.rama_tenido_turno), RAMA_TENIDO_TURNO_OPTIONS)}
                    </select>
                </td>
                <td>
                    <input class="table-input" type="text" value="${TintoreriaUtils.escapeHtml(TintoreriaUtils.lastPassValue(record.rama_tenido_operario))}" readonly>
                </td>
                <td>
                    <select class="table-select" tabindex="-1" style="pointer-events:none; appearance:none; -webkit-appearance:none;">
                        ${optionMarkup(TintoreriaUtils.lastPassValue(record.rama_tenido_maquina), RAMA_TENIDO_MAQUINA_OPTIONS)}
                    </select>
                </td>
                <td>
                    <select class="table-select" tabindex="-1" style="pointer-events:none; appearance:none; -webkit-appearance:none;">
                        ${optionMarkup(TintoreriaUtils.lastPassValue(record.rama_tenido_proceso), RAMA_TENIDO_PROCESO_OPTIONS)}
                    </select>
                </td>
                <td>
                    <input class="table-input" type="text" value="${TintoreriaUtils.escapeHtml(TintoreriaUtils.lastPassValue(record.rama_tenido_inspector))}" readonly>
                </td>
                <td>
                    <input class="table-input" type="text" value="${TintoreriaUtils.escapeHtml(TintoreriaUtils.lastPassValue(record.rama_tenido_supervisor))}" readonly>
                </td>
                <td>
    ...
    function syncVisibleSubtabSummary() {
        ...
        const countNode = document.getElementById(isProg ? 'count-rama-tenido-prog' : 'count-rama-tenido-xprog');
        const summaryNode = document.getElementById(isProg ? 'summary-rama-tenido-prog' : 'summary-rama-tenido-xprog');
    ...
    // Celda Inicio (subtab Por procesar): muestra el inicio del ULTIMO pase registrado.
    function renderInicioMarkup(record) {
        const label = TintoreriaUtils.formatProcessDateTimeLabel(TintoreriaUtils.lastPassValue(record.rama_tenido_inicio));
        if (!label) {
            return '<span class="cell-text">—</span>';
        }
        return `<span class="process-pill process-pill-info" title="${TintoreriaUtils.escapeHtml(label)}">${TintoreriaUtils.escapeHtml(label)}</span>`;
    }
    ...
    function renderTable(records, state) {
        ...
        if (!filtered.length) {
            ...
            TintoreriaApp.refreshViewDecorations('rama-tenido');
            return;
        }
                ...
                <tr${TintoreriaUtils.isUrgentPriority(record.rama_tenido_p) ? ' class="urgent-row"' : ''}>
                    ...
                    <td>
                        ${hasOpenPass(record)
                            ...
                            : `<select class="table-select" data-record-id="${TintoreriaUtils.escapeHtml(record.id_registro)}" data-field="rama_tenido_estado">
                                ${(() => {
                                    const currentState = normalizeRamaTenidoState(record);
                                    const opts = [...RAMA_TENIDO_ESTADO_OPTIONS];
                                    if (currentState && !opts.includes(currentState)) opts.unshift(currentState);
                                    return opts.map((v) => {
                                        const label = v === 'X PROG' ? 'X PROCESAR' : (v || 'Selec');
                                        const selected = currentState === v ? 'selected' : '';
    ...
    async function handleEditableChange(event) {
        ...
        if (currentFilter === 'PROG' && field !== 'rama_tenido_estado') {
            return;
        }
    ...
    function syncDurationTimer(records) {
        const shouldRun = getEligibleRecords(records).some((record) => hasOpenPass(record));
    ...
    TintoreriaApp.registerView('rama-tenido', {
        ...
        locateRecord(record, state) {
            ...
            if (['PROG', 'OK'].includes(rtState)) {
                const allProg = getEligibleRecords(state.records || [])
                    .filter((r) => ['PROG', 'OK'].includes(normalizeRamaTenidoState(r)));
                const visible = TintoreriaProcessedWindow.filterToWindow('rama-tenido', allProg, getProcessedDate);
