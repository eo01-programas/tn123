(() => {
    ...
    function renderSubtabCounts(records) {
        ...
        document.getElementById('count-rama-tenido-prog').textContent = `${new Set(progRecords.map((r) => TintoreriaUtils.formatOpPartida(r.op_tela, r.partida))).size} ptds`;
        ...
        if (isProcesado) {
            ...
            tbody.innerHTML = filtered.map((record) => `
                <tr${TintoreriaUtils.isUrgentPriority(record.rama_tenido_p) ? ' class="urgent-row"' : ''}>
                    ...
                    <td>
                        <select class="table-select" data-record-id="${TintoreriaUtils.escapeHtml(record.id_registro)}" data-field="rama_tenido_estado">
                            ${RAMA_TENIDO_ESTADO_OPTIONS.map((v) => {
                                const label = v === 'X PROG' ? 'X PROCESAR' : (v || 'Selec');
                                const selected = normalizeRamaTenidoState(record) === v ? 'selected' : '';
