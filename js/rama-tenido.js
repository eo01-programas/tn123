(() => {
    ...
    // P | cliente | OP-PTDA | color | cod_art | articulo | kg | Turno | Oper | MAQ | Proceso | Insp | Superv | Inicio | Fin | Status
    const RAMA_TENIDO_WIDTHS = [37, 37, 74, 87, 99, 93, 62, 62, 87, 68, 81, 74, 74, 62, 62, 68];
    const DETAIL_FIELDS = [
        'rama_tenido_ancho',
    ...
    function renderSubtabCounts(records) {
        ...
        if (colgroup) colgroup.innerHTML = RAMA_TENIDO_WIDTHS.map(w => `<col style="width:${w}px">`).join('');
    }
