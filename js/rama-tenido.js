(() => {
    ...
    // Por procesar (12): P | cliente | OP-PTDA | color | cod_art | articulo | kg | Turno | MAQ | Proceso | Inicio | Status
    const RAMA_TENIDO_WIDTHS = [37, 72, 108, 100, 90, 220, 68, 60, 72, 85, 75, 95];
    // Procesado (14): Fin | cliente | OP-PTDA | color | cod_art | articulo | kg | Turno | Oper | MAQ | Proceso | Insp | Superv | Status
    const RAMA_TENIDO_WIDTHS_PROCESADO = [68, 52, 99, 87, 79, 180, 62, 62, 87, 62, 81, 74, 74, 68];
    const DETAIL_FIELDS = [
        'rama_tenido_ancho',
    ...
    function getFilteredRecords(records) {
        ...
        if (currentFilter === 'PROG') {
            const sorted = eligible
                ...
                .sort((a, b) => {
                    const dateA = TintoreriaUtils.parseDateish(a.rama_tenido_fin);
                    const dateB = TintoreriaUtils.parseDateish(b.rama_tenido_fin);
    ...
    function getProcessedDate(record) {
        return getAdjustedRamaTenidoDate(record && record.rama_tenido_fin);
    }

    function renderRowProcesado(record) {
        const adjusted = getAdjustedRamaTenidoDate(record.rama_tenido_fin);
        const finDate = TintoreriaUtils.formatDateDayMonth(adjusted) || '--';
        const inicioLabel = TintoreriaUtils.formatProcessDateTimeLabel(record.rama_tenido_inicio) || '--';
        const finLabel = TintoreriaUtils.formatProcessDateTimeLabel(record.rama_tenido_fin) || '--';
        const elapsed = TintoreriaUtils.formatElapsedTime(record.rama_tenido_inicio, record.rama_tenido_fin) || '--:--';
        ...
        return `
            <tr${TintoreriaUtils.isUrgentPriority(record.rama_tenido_p) ? ' class="urgent-row"' : ''}>
                ...
                <td>
                    <input class="table-input" type="text" value="${TintoreriaUtils.escapeHtml(record.rama_tenido_operario || '')}" readonly>
                </td>
                <td>
                    <select class="table-select" tabindex="-1" style="pointer-events:none; appearance:none; -webkit-appearance:none;">
                ...
                <td>
                    <input class="table-input" type="text" value="${TintoreriaUtils.escapeHtml(record.rama_tenido_inspector || '')}" readonly>
                </td>
                <td>
                    <input class="table-input" type="text" value="${TintoreriaUtils.escapeHtml(record.rama_tenido_supervisor || '')}" readonly>
                </td>
                <td>
        ...
        if (!filtered.length) {
            tbody.innerHTML = `
                <tr class="empty-state">
                    <td colspan="${isProcesado ? 14 : 12}">No hay filas para este subtab.</td>
                </tr>
            `;
        ...
        if (isProcesado) {
            ...
            tbody.innerHTML = filtered.map((record) => `
                <tr${TintoreriaUtils.isUrgentPriority(record.rama_tenido_p) ? ' class="urgent-row"' : ''}>
                    ...
                    <td>
                        ${record.rama_tenido_inicio
                            ? `<span class="process-pill process-pill-info" title="${TintoreriaUtils.escapeHtml(TintoreriaUtils.formatProcessDateTimeLabel(record.rama_tenido_inicio) || '')}">${TintoreriaUtils.escapeHtml(TintoreriaUtils.formatProcessDateTimeLabel(record.rama_tenido_inicio) || '')}</span>`
