(() => {
    ...
    function getQualityReadonlyControlAttrs(isReadonly) {
        return isReadonly
            ? ' disabled aria-disabled="true" tabindex="-1" style="appearance:none; -webkit-appearance:none;"'
            : '';
    }
    ...
    function getVisibleRecords(records, filter = currentFilter, options = {}) {
        const { ignoreRejectedWindow = false } = options;

        if (filter === 'REJECTED') {
            const rejectedRecords = getRejectedRecords(records);
            if (ignoreRejectedWindow) {
                return rejectedRecords;
            }

            return TintoreriaProcessedWindow.filterToWindow(
                'calidad', rejectedRecords, getFechaRegistroDate, AUDITADAS_WINDOW_DAYS
            );
        }
    ...
    function getFinishExportLabel(record, filter) {
        ...
        const isFrozenBySituacion = Boolean(record.cal_situacion && record.cal_situacion_fecha);
        const endRef = record.calidad_fin
            ? record.calidad_fin
            : (isFrozenBySituacion ? (TintoreriaUtils.parseDateish(record.cal_situacion_fecha) || new Date()) : new Date());

        return TintoreriaUtils.formatElapsedDaysNoDomingos(record.calidad_inicio, endRef) || '0.5días';
    }
    ...
    function renderSubtabCounts(records) {
        ...
        if (activeCount) {
            activeCount.textContent = `${new Set(activeRecords.map((r) => TintoreriaUtils.formatOpPartida(r.op_tela, r.partida))).size} ptds`;
        }

        if (rejectedCount) {
            rejectedCount.textContent = `${new Set(rejectedRecords.map((r) => TintoreriaUtils.formatOpPartida(r.op_tela, r.partida))).size} ptds`;
        }
    ...
    function renderFinishMarkup(record, isReadonly = false) {
        ...
        const durationLabel = TintoreriaUtils.formatElapsedDaysNoDomingos(record.calidad_inicio, endRef) || '0.5días';
    ...
    function renderTable(records, state) {
        ...
        const hasQualityLookup = Boolean(normalizeQualityLookupQuery(qualityLookupQuery));
        const hasGlobalLookup = Boolean(state && state.activeSearch && state.activeSearch.viewId === 'calidad');
        const filtered = filterRecordsForQualityLookup(
            TintoreriaUtils.filterRecordsForSearch(
                getVisibleRecords(records, currentFilter, { ignoreRejectedWindow: hasQualityLookup || hasGlobalLookup }),
                state,
                'calidad'
            )
        );
        ...
        if (currentFilter === 'ACTIVE') {
            const now = Date.now();
            filtered.sort((a, b) => {
                const aFrozen = Boolean(String(a.cal_situacion || '').trim());
                const bFrozen = Boolean(String(b.cal_situacion || '').trim());
        ...
        tbody.innerHTML = filtered.map((record) => {
            ...
            if (currentFilter === 'ACTIVE' && String(record.cal_situacion || '').trim() === 'xTela cuerpo') rowClasses.push('xtela-cuerpo-error-row');
            ...
            const tdOpptda = `<td><div class="quality-op-cell">${renderInfoButtonMarkup(record)}<span class="cell-text code-text">${TintoreriaUtils.escapeHtml(TintoreriaUtils.formatOpPartida(record.op_tela, record.partida))}</span></div></td>`;
    ...
    async function handleEditableChange(event) {
        ...
        const fieldUpdates = { [field]: nextValue };
        if (field === 'cal_situacion') {
            const oldSituacion = String(currentRecord.cal_situacion || '').trim();
            const newSituacion = String(nextValue || '').trim();
            if (newSituacion && !oldSituacion) {
                fieldUpdates.cal_situacion_fecha = TintoreriaUtils.formatProcessDateTime(new Date());
            } else if (!newSituacion) {
                fieldUpdates.cal_situacion_fecha = '';
            }
        }

        try {
            await TintoreriaApp.saveRecordChanges(recordId, fieldUpdates, { silent: true });
        } catch (error) {
    ...
    function updateSubtabSummaryFromDOM() {
        ...
        if (currentFilter === 'REJECTED') {
            ...
            if (countEl) countEl.textContent = `${uniqueOpCount} ptds`;
    ...
    function getInfoModalElements() {
        return {
            ...
            fourPointBtn: document.getElementById('calidad-info-four-point')
        };
    }
    ...
    function openInfoModal(record) {
        ...
        currentInfoRecordId = record.id_registro;
    ...
    function openFourPointAuditPage() {
        if (!currentInfoRecordId) {
            return;
        }

        const record = TintoreriaApp.findRecord(currentInfoRecordId);
        if (!record) {
            TintoreriaApp.showToast('No se encontro el registro de calidad.', 'error', 'Auditoria no disponible');
            return;
        }

        try {
            sessionStorage.setItem('tintoreria_four_point_audit_record', JSON.stringify(record));
        } catch (error) {
            console.warn('No se pudo guardar el registro de auditoria 4 puntos.', error);
        }

        window.location.href = './auditoria-4-puntos.html';
    }
    ...
    async function handleRejectSave() {
        ...
        const currentRecord = TintoreriaApp.findRecord(currentRejectRecordId);
        const formData = new FormData(form);
        const updates = {
            ...buildEstadoUpdatesByRuta(currentRecord && currentRecord.ruta),
    ...
    function init() {
        ...
        const { modal: infoModal, closeBtn: infoCloseBtn, fourPointBtn } = getInfoModalElements();
        if (infoCloseBtn) infoCloseBtn.addEventListener('click', closeInfoModal);
        if (fourPointBtn) fourPointBtn.addEventListener('click', openFourPointAuditPage);
        if (infoModal) {
