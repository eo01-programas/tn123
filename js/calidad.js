(() => {
    ...
    function renderTableHead() {
        ...
        const headers = isRejected
            ? ['Fin', 'cliente', 'OP-PTDA', 'cod_color', 'color', 'cod_art', 'articulo', 'kg(crudo)', '#rollos/cntd', 'Auditor', 'Supervisor', 'Turno', 'Inicio', 'Status']
            : ['cliente', 'OP-PTDA', 'cod_color', 'color', 'cod_art', 'articulo', 'kg(crudo)', '#rollos/cntd', 'Auditor', 'Turno', 'Inicio', 'Fin', 'Status'];
        // Pixel widths per column — different for each filter since Fin moves to col 1 in AUDITADAS
        // and OP-PTDA needs extra space in AUDITADAS because of the eye button (36px + gap)
        // ACTIVE has 13 columns (Supervisor hidden); AUDITADAS has 14 columns
        const widths = isRejected
            ? [88, 70, 110, 66, 96, 66, 132, 62, 68, 84, 90, 60, 80, 110]
    ...
    function renderTable(records, state) {
        ...
        if (!filtered.length) {
            ...
            tbody.innerHTML = `<tr class="empty-state"><td colspan="${currentFilter === 'REJECTED' ? 14 : 13}">${emptyLabel}</td></tr>`;
        ...
        tbody.innerHTML = filtered.map((record) => {
            ...
            const cells = currentFilter === 'REJECTED'
                ? [tdFin, tdCliente, tdOpptda, tdCodColor, tdColor, tdCodArt, tdArticulo, tdKg, tdRollos, tdAuditor, tdSupervisor, tdTurno, tdInicio, tdStatus]
