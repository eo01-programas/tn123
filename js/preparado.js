(() => {
    ...
    // X PROG (13): P | F_ing_crudo | cliente | tela | OP-PTDA | color | articulo | kg | ruta | Turno | Tipo | Inicio | Status
    const PREPARADO_XPROG_WIDTHS = [36, 70, 70, 50, 78, 130, 330, 56, 67, 73, 118, 75, 101];
    // PROG  (13): F_preparado | cliente | tela | OP-PTDA | color | articulo | kg | ruta | Turno | Responsable | Equipo | Tipo | Status
    const PREPARADO_PROG_WIDTHS  = [67, 56, 50, 78, 101, 280, 56, 56, 45, 112, 90, 101, 45];
    ...
    // ¿Tiene un pase abierto? (mas inicios que fines registrados por el mobile)
    function hasOpenPass(record) {
        const inicios = TintoreriaUtils.parsePassValues(record.preparado_inicio).filter(Boolean).length;
        const fines = TintoreriaUtils.parsePassValues(record.preparado_fin).filter(Boolean).length;
    ...
    function getFilteredRecords(records) {
        ...
        if (currentFilter === 'PROG') {
            const sorted = TintoreriaUtils.sortRecordsByPriority(
                ...
                const dateA = TintoreriaUtils.parseDateish(TintoreriaUtils.lastPassValue(a.preparado_fin));
                const dateB = TintoreriaUtils.parseDateish(TintoreriaUtils.lastPassValue(b.preparado_fin));
                const timeA = dateA ? dateA.getTime() : 0;
                const timeB = dateB ? dateB.getTime() : 0;
                return timeB - timeA;
            });
            return TintoreriaProcessedWindow.filterToWindow('preparado', sorted, getProcessedDate);
        }
    ...
    function getProcessedDate(record) {
        return getAdjustedPreparadoDate(record ? TintoreriaUtils.lastPassValue(record.preparado_fin) : null);
    }

    function renderFPreparadoCell(record) {
        const lastInicio = TintoreriaUtils.lastPassValue(record.preparado_inicio);
        const lastFin = TintoreriaUtils.lastPassValue(record.preparado_fin);
        const adjusted = getAdjustedPreparadoDate(lastFin);
        const finLabel = TintoreriaUtils.formatDateDayMonth(adjusted);
        const finFull = TintoreriaUtils.formatProcessDateTimeLabel(lastFin) || '--';
        const inicioFull = TintoreriaUtils.formatProcessDateTimeLabel(lastInicio) || '--';
    ...
    function renderSubtabCounts(records) {
        ...
        document.getElementById('count-preparado-prog').textContent = `${new Set(progWindowed.map((r) => TintoreriaUtils.formatOpPartida(r.op_tela, r.partida))).size} ptds`;
        document.getElementById('summary-preparado-xprog').textContent = TintoreriaUtils.formatSubtabSummary(xprogRecords);
        document.getElementById('summary-preparado-prog').textContent = TintoreriaUtils.formatSubtabSummary(progWindowed);
    }
    ...
    // Celda Inicio (subtab X PROG): muestra el inicio del ULTIMO pase registrado.
    function renderInicioMarkup(record) {
        const label = TintoreriaUtils.formatProcessDateTimeLabel(TintoreriaUtils.lastPassValue(record.preparado_inicio));
        if (!label) {
    ...
    function renderTable(records, state) {
        ...
        tbody.innerHTML = filtered.map((record) => `
            <tr${TintoreriaUtils.isUrgentPriority(record.preparado_p) ? ' class="urgent-row"' : ''}>
                ...
                <td>
                    <select class="table-select" ${readOnly ? 'tabindex="-1" style="pointer-events:none; appearance:none; -webkit-appearance:none;"' : `data-record-id="${TintoreriaUtils.escapeHtml(record.id_registro)}" data-field="preparado_turno"`}>
                ...
                <td>
                    <select class="table-select" ${readOnly ? 'tabindex="-1" style="pointer-events:none; appearance:none; -webkit-appearance:none;"' : `data-record-id="${TintoreriaUtils.escapeHtml(record.id_registro)}" data-field="preparado_tipo"`}>
                ...
                <td>${currentFilter !== 'PROG' && hasOpenPass(record)
                    ...
                    : renderEstadoMarkup(record, readOnly)
                }</td>
            </tr>
        `).join('');
    ...
    async function handleEditableChange(event) {
        ...
        if (currentFilter === 'PROG' && field !== 'preparado_estado') {
            return;
        }
        if (!recordId || !field) {
            return;
        ...
        if (field === 'preparado_estado' && nextValue === 'OK') {
            ...
            // Misma regla que el mobile: DOBLADO/DOB-REPROCESO son pases parciales,
            // no cierran el preparado hasta registrar un tipo de cierre (ej. COSIDO).
            if (hasOnlyPartialPreparadoTipos(currentRecord)) {
                target.value = normalizePreparadoState(currentRecord);
                TintoreriaApp.showToast('DOBLADO/DOB-REPROCESO no completa el preparado: falta el pase de cosido para marcar OK.', 'error', 'Preparado incompleto');
                return;
            }
    ...
    TintoreriaApp.registerView('preparado', {
        init,
        processedDate: { columnLabel: 'F_PREPARADO', getDate: getProcessedDate },
