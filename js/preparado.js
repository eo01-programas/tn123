(() => {
    ...
    // X PROG (11): P | F_ing_crudo | cliente | OP-PTDA | color | articulo | kg | ruta | Turno | Tipo | Status
    const PREPARADO_XPROG_WIDTHS = [36, 67, 90, 78, 168, 290, 56, 67, 73, 101, 101];
    ...
    function updateColumnVisibility() {
        ...
        if (inicioHeader) inicioHeader.hidden = true;
    ...
    function renderEstadoMarkup(record) {
        ...
        return `<select class="table-select" data-record-id="${TintoreriaUtils.escapeHtml(record.id_registro)}" data-field="preparado_estado">${statusOptions}</select>`;
    }
    ...
    function renderTable(records, state) {
        ...
        if (!filtered.length) {
            tbody.innerHTML = `
                <tr class="empty-state">
                    <td colspan="${currentFilter === 'PROG' ? 12 : 11}">No hay filas para este subtab.</td>
                </tr>
            `;
        ...
        tbody.innerHTML = filtered.map((record) => `
            <tr${TintoreriaUtils.isUrgentPriority(record.preparado_p) ? ' class="urgent-row"' : ''}>
                ...
                ${currentFilter === 'PROG' ? `
                <td>
                    <input class="table-input" type="text" value="${TintoreriaUtils.escapeHtml(record.preparado_supervisor || '')}" data-record-id="${TintoreriaUtils.escapeHtml(record.id_registro)}" data-field="preparado_supervisor">
                </td>
                <td>
                ...
                <td>${renderEstadoMarkup(record)}</td>
            </tr>
        `).join('');
