(() => {
    const STOCK_AREAS = [
        ...
        },
        {
            ...
            isEligible(record) {
                return String(record.tenido_estado || '').trim() === 'OK'
                    && normalizeProcessState(record.secado_estado) !== 'OK';
            },
            isProgrammed(record) {
                return normalizeProcessState(record.secado_estado) === 'PROG';
            }
        },
        {
            id: 'rama-tenido',
        ...
        },
        {
            ...
            isProgrammed(record) {
                return Boolean(String(record.calidad_inicio || '').trim());
            }
        },
        {
            id: 'embalaje',
    ...
    const CALIDAD_REJECTED_STATES = new Set(['RECHAZADO', '1ER RECHAZO', '2DO RECHAZO', '3ER RECHAZO', '4TO RECHAZO']);
    ...
    function isCalidadRejected(record) {
        return CALIDAD_REJECTED_STATES.has(normalizeCalidadState(record.calidad_estado));
    }
    ...
    function buildStockDataset(records) {
        ...
        return STOCK_AREAS.map((area) => {
            ...
            } else if (area.id === 'rechazos-calidad') {
                rejectedRecords = eligibleRecords;
                porProgramarRecords = [];
                programadoRecords = [];
