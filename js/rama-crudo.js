(() => {
    ...
    function hasOpenPass(record) {
        const inicios = TintoreriaUtils.parsePassValues(record.rama_crudo_inicio).filter(Boolean).length;
        const fines = TintoreriaUtils.parsePassValues(record.rama_crudo_fin).filter(Boolean).length;
    ...
    function getFilteredRecords(records) {
        ...
        if (currentFilter === 'PROG') {
            const sorted = eligible
                ...
                .sort((a, b) => {
                    const dateA = TintoreriaUtils.parseDateish(TintoreriaUtils.lastPassValue(a.rama_crudo_fin));
                    const dateB = TintoreriaUtils.parseDateish(TintoreriaUtils.lastPassValue(b.rama_crudo_fin));
    ...
    function getProcessedDate(record) {
        return getAdjustedRamaCrudoDate(record ? TintoreriaUtils.lastPassValue(record.rama_crudo_fin) : null);
    }

    function renderRowProcesado(record) {
        const lastInicio = TintoreriaUtils.lastPassValue(record.rama_crudo_inicio);
        const lastFin = TintoreriaUtils.lastPassValue(record.rama_crudo_fin);
        const adjusted = getAdjustedRamaCrudoDate(lastFin);
    ...
    function renderInicioMarkup(record) {
        const label = TintoreriaUtils.formatProcessDateTimeLabel(TintoreriaUtils.lastPassValue(record.rama_crudo_inicio));
        if (!label) {
