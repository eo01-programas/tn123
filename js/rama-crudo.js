(() => {
    ...
    // Por procesar (12): P | cliente | OP-PTDA | color | cod_art | articulo | kg | Turno | MAQ | ruta | Inicio | Status
    const RAMA_CRUDO_WIDTHS = [37, 72, 108, 100, 90, 220, 68, 60, 72, 85, 75, 95];
    ...
    function renderSubtabCounts(records) {
        ...
        document.getElementById('count-rama-crudo-prog').textContent = `${new Set(progRecords.map((r) => TintoreriaUtils.formatOpPartida(r.op_tela, r.partida))).size} ptds`;
    ...
    const RAMA_CRUDO_WIDTHS_PROCESADO = [68, 52, 99, 87, 79, 93, 62, 62, 87, 62, 81, 74, 74, 68];
