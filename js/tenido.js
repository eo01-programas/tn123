(() => {
    ...
    // P | cliente | OP-PTDA | cod_color | color | kg | Turno | Oper | MAQ | Proceso | Tipo_proceso | Kg_ten | Control | Superv | Inicio | Fin | Status
    const TENIDO_WIDTHS = [37, 52, 77, 68, 87, 62, 56, 81, 68, 93, 105, 68, 81, 81, 65, 50, 56];
    ...
    function renderSubtabCounts(records) {
        ...
        document.getElementById('count-tenido-xprog').textContent = `${new Set(xprogRecords.map((r) => TintoreriaUtils.formatOpPartida(r.op_tela, r.partida))).size} ptds`;
        document.getElementById('count-tenido-prog').textContent = `${new Set(progRecords.map((r) => TintoreriaUtils.formatOpPartida(r.op_tela, r.partida))).size} ptds`;
        ...
        const colgroup = document.getElementById('colgroup-tenido');
        if (colgroup) colgroup.innerHTML = TENIDO_WIDTHS.map(w => `<col style="width:${w}px">`).join('');
    }
    ...
    function renderTable(records, state) {
        ...
        if (!filtered.length) {
            ...
            TintoreriaApp.refreshViewDecorations('tenido');
            return;
        }
