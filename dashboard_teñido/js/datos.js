...
const Datos = (() => {
  ...
  function cargarArticulosUnicos(filas) {
    ...
    const articulos = [];
    (filas || []).forEach(f => {
      const codRaw = Utils.texto(f['Cod. Art.']);
      const descRaw = Utils.texto(f['Descripcion Art.']);
      const codigos = Utils.splitPipe(codRaw);
      const descripciones = descRaw.includes('|')
        ? Utils.splitPipe(descRaw)
        : [descRaw];

      const total = Math.max(codigos.length, descripciones.length);
      for (let i = 0; i < total; i++) {
        articulos.push({
          codigo: Utils.texto(codigos[i]).replace(/^0+(?=\d)/, ''),
          descripcion: Utils.texto(descripciones[i])
        });
      }
    });
    Estado.articulosUnicos = articulos.filter(a => {
  ...
  /* Tono del color (vista DETALLE / tablero): la hoja no trae la
     columna, se deduce del texto de "Colores" con CONFIG.MAPA_TONOS
     (primera coincidencia gana, ver el comentario en config.js). */
  function clasificarTono(color) {
    const c = Utils.clave(color);
    if (!c) return CONFIG.TONO_POR_DEFECTO;
    for (const regla of CONFIG.MAPA_TONOS)
      if (c.includes(Utils.clave(regla.contiene))) return regla.tono;
    return CONFIG.TONO_POR_DEFECTO;
  }
  ...
  function normalizarFila(f) {
    ...
    /* Códigos divididos sin ceros a la izquierda ("00030199" -> "30199"):
       así el panel de Artículo no muestra el mismo código dos veces y el
       filtro compara por igualdad. codArt conserva el valor combinado. */
    const codArts     = Utils.splitPipe(codArt)
      .map(c => c.replace(/^0+(?=\d)/, ''));
    const descArtTienePipe = String(
      descArtRaw === null || descArtRaw === undefined ? '' : descArtRaw
    ).includes('|');
    const descArts = Utils.splitDescripcionArt(descArtRaw);
    const descArt = descArts.length
      ? descArts.join(' | ')
      : Utils.texto(descArtRaw);
    const descArtsConSeparadorExplicito = descArtTienePipe;
    ...
    const reg = {
      ...
      codArt, codArts,
      descArt, descArts, descArtsConSeparadorExplicito,
      colores, coloresList,
      tipoRecetas,
      tipoRecetaClase,
      ...
      articulo:     extraerArticulo(descArt),
    ...
    const base = Estado.registros.filter(r => {
      ...
      if (usar('articulos') && F.articulos && F.articulos.size) {
        ...
        /* El código se compara por IGUALDAD EXACTA contra cada código
           separado internamente desde Cod. Art., ignorando ceros a la
           izquierda: "30199" coincide con
