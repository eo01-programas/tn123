...
const Utils = (() => {
  ...
  /* ---------- Splits de campos compuestos ----------
     "Cod. Art.", "Descripcion Art." y "Colores" usan " | ". */
  ...
  /* El pipe es el formato vigente. El doble espacio se conserva solo
     para poder leer filas historicas creadas antes del cambio. */
  function splitDescripcionArt(v) {
    const original = v === null || v === undefined ? '' : String(v);
    return original.includes('|')
      ? splitPipe(original)
      : splitDobleEspacio(original);
  }
  ...
  return {
    ...
    splitPipe, splitDobleEspacio, splitDescripcionArt, splitPartidas,
