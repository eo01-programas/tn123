/* ============================================================
   UI.JS — KPIs, tabla de detalle, ficha rápida de OP-Partida
   y resumen del periodo
   ============================================================ */
...
const UI = (() => {
  ...
  /* ---------- Tabla de detalle de reprocesos ---------- */
  function tablaDetalle(reprocesos) {
    const cuerpo = $('tbodyDetalle');
    if (!reprocesos.length) {
      cuerpo.innerHTML =
        '<tr><td colspan="10" class="sc8-vacio">Sin reprocesos para los filtros seleccionados.</td></tr>';
    } else {
      cuerpo.innerHTML = reprocesos.map(r => `
        <tr data-partida="${Utils.escapeHtml(r.partidas[0] || '')}">
          <td class="sc8-link" title="${Utils.escapeHtml(r.opPartida)}">${Utils.escapeHtml(r.opPartidaCorta || r.opPartida || '—')}</td>
          <td>${Utils.escapeHtml(r.fechaTxt)}</td>
          <td>${Utils.escapeHtml(r.clienteCorto)}</td>
          <td title="${Utils.escapeHtml(r.descArt)}">${Utils.escapeHtml(r.articulo)}</td>
          <td>${Utils.escapeHtml(Utils.truncar(r.color1, 16))}</td>
          <td>${Utils.escapeHtml(r.maqOrigen || '—')}</td>
          <td>${Utils.escapeHtml(r.defecto)}</td>
          <td title="${Utils.escapeHtml(r.tipoProcesos)}">${Utils.escapeHtml(Utils.truncar(r.tipoProcesos, 24))}</td>
          <td>${Utils.escapeHtml(r.maquina)}</td>
          <td>${badgeEstado(r)}</td>
        </tr>`).join('');
    }
    $('totalRegistros').textContent = 'Total registros: ' + reprocesos.length;

    // Clic en una fila -> carga la ficha rápida.
    cuerpo.querySelectorAll('tr[data-partida]').forEach(tr => {
      tr.addEventListener('click', () => {
        const p = tr.getAttribute('data-partida');
        if (p) { $('buscarPartida').value = p; fichaRapida(p); }
      });
    });
  }
  ...
  function renderizar(modelo) {
    ...
    tablaDetalle(modelo.reprocesos);
    resumen(modelo.kpis);
